...
func progressUpdate(steps: Int, goal: Int) {
    let dblSteps = Double(steps)
    let dblGoal = Double(goal)
    
    if dblSteps < dblGoal * 0.1 {
        print("You're off to a good start.")
    } else if dblSteps < dblGoal * 0.5 {
        print("You're almost halfway there!")
    } else if dblSteps < dblGoal * 0.9 {
...
progressUpdate(steps: 1, goal: 100)
...
func pacing(currentDistance: Double, totalDistance: Double, currentTime: Double, goalTime: Double) {
    ...
    let projectedDistance = pace * goalTime
    
    print(projectedDistance >= totalDistance ? "Keep it up!" : "You've got to push it just a bit harder!")
}

pacing(currentDistance: 10, totalDistance: 100, currentTime: 15, goalTime: 100)
